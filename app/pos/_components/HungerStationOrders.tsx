...
export default function HungerStationOrders({ open, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    fetchOrders()
    const ch = supabase.channel('hs-orders')
  ...
  async function handleAccept(order: DeliveryOrder) {
    ...
    try {
      ...
      // Callback to HungerStation
      if (order.platform_order_id) {
        fetch('/api/delivery/accept', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ platform: 'hungerstation', platformOrderId: order.platform_order_id }),
        }).catch(() => {})
      }
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0"
              ...
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: HS_ORANGE }}>
                  <span className="text-white font-black text-xs">HS</span>
                </div>
                <div>
                  <p className="font-black text-white text-sm">HungerStation</p>
            ...
            <div className="flex-1 overflow-y-auto">
              {loading && orders.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
                  ...
                  <p className="text-gray-400 text-sm">Pesanan HungerStation akan muncul otomatis</p>
                </div>
              ) : (
                <div className="p-4 space-y-3">
                  {orders.map(order => (
                    ...
                    >
                      <div className="h-1" style={{ background: HS_ORANGE }} />
                      <div className="p-4">
                        {/* Meta */}
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="text-right">
                            <p className="font-black text-sm" style={{ color: HS_ORANGE }}>
                        ...
                        {order.delivery_address && (
                          <div className="bg-orange-50 rounded-lg px-3 py-2 mb-3">
                            <p className="text-[10px] font-black text-orange-500 uppercase tracking-wide mb-0.5">Alamat Pengiriman</p>
                        ...
                        <div className="space-y-1.5 mb-4">
                          {order.order_items.map(item => (
                            <div key={item.id} className="flex items-start gap-2">
                              <span className="font-black text-sm w-6 flex-shrink-0" style={{ color: HS_ORANGE }}>
                                {item.qty}x
                              </span>
                        ...
                        {order.status === 'pending' ? (
                          <motion.button
                            ...
                            style={{ background: HS_ORANGE, boxShadow: '0 4px 16px rgba(255,96,0,0.35)' }}
                          >
                            <Bell size={14} />
